(()=>{
  ...
  function ensureUtilityRow(result,creatorBox,skinBox){
    ...
    if(skinBox&&skinBox.parentElement!==row){skinBox.style.marginTop='0';skinBox.style.height='100%';row.appendChild(skinBox)}
    return row;
  }
  function enhance(){const result=document.getElementById('result');if(!result?.classList.contains('show'))return;const shareBox=result.querySelector('.sharebox'),skinBox=result.querySelector('.skinbox');if(!shareBox)return;const creatorBox=ensureCreatorBox(result,shareBox);const row=ensureUtilityRow(result,creatorBox,skinBox);let qrBox=result.querySelector('.qrbox');if(!qrBox){qrBox=document.createElement('section');qrBox.className='qrbox';qrBox.innerHTML='<div><div class="section-kicker">扫码测试</div><div class="qr-title">看看朋友是牛来还是牛马</div><div class="qr-copy">长按保存或扫码进入测试</div></div><img src="images/test-qr.png" alt="牛马人格测试网址二维码" width="600" height="600">'}qrBox.style.marginTop='0';qrBox.style.height='100%';qrBox.style.minWidth='0';if(qrBox.parentElement!==row)row.prepend(qrBox);const qrImg=qrBox.querySelector('img');if(qrImg){qrImg.style.width='min(104px,38%)';qrImg.style.height='auto'}const imageButton=document.getElementById('share'),linkButton=document.getElementById('friend');if(imageButton&&!imageButton.dataset.longCard){imageButton.dataset.longCard='1';imageButton.textContent='分享结果长图';imageButton.onclick=shareCard}if(linkButton&&!linkButton.dataset.directLink){linkButton.dataset.directLink='1';linkButton.textContent='分享测试网址';linkButton.onclick=shareLink}}
