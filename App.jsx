...
const INGREDIENT_LIBRARY = [
  ...
  // ---- 抗氧化体系 ----
  { name: "维生素C衍生物 (Ascorbyl Glucoside/Palmitate/3-O-Ethyl Ascorbic Acid)", system: "抗氧化体系", func: "中和自由基,抑制黑色素合成,比左旋VC稳定刺激低", goodFor: ["photodamage", "pigmentation"] },
  { name: "阿魏酸 (Ferulic Acid)", system: "抗氧化体系", func: "增强维C稳定性,协同抗氧化", goodFor: ["photodamage"] },
  { name: "咖啡酸 (Caffeic Acid)", system: "抗氧化体系", func: "多酚类抗氧化,常与阿魏酸复配", goodFor: ["photodamage"] },
  { name: "生育酚/维E (Tocopherol/Tocopheryl Acetate)", system: "抗氧化体系", func: "经典脂溶性抗氧化剂,兼具轻度保湿", goodFor: ["photodamage", "barrier"] },
  { name: "藻类提取物 (Algae Extract)", system: "抗氧化体系", func: "富含矿物质和多糖,兼具抗氧化和舒缓", goodFor: ["photodamage", "barrier"] },
  { name: "蕨类提取物 (Polypodium Leucotomos)", system: "抗氧化体系", func: "口服/外用均有文献支持的光保护抗氧化成分", goodFor: ["photodamage"] },
...
function App() {
  ...
  const suitability = symptomResults.length > 0 ? mergeSuitability(symptomResults) : null;
      ...
      <div style={{ width: "100%", maxWidth: 460 }}>
        ...
        {screen === "intro" && (
          <div style={{ paddingTop: 40 }}>
            <Eyebrow>皮肤鉴别诊断 · DEMO 原型</Eyebrow>
            <h1 style={{ fontFamily: "'Newsreader', serif", fontSize: 34, fontWeight: 500, lineHeight: 1.25, marginBottom: 16, marginTop: 0 }}>
              同一个症状,
              <br />
              不一定是同一个原因。
            </h1>
            <p style={{ fontSize: 14.5, color: "#5C574D", lineHeight: 1.7, marginBottom: 28 }}>
              我们用结构化追问,像鉴别诊断一样收窄可能的病因,再给出带置信度、带证据拆解的判断——而不是一个武断的结论。
            </p>
            <div style={{ border: `1px solid ${LINE}`, borderRadius: 10, padding: "16px 18px", marginBottom: 28, background: "#fff" }}>
              <div style={{ display: "flex", gap: 10, fontSize: 13, color: "#5C574D", lineHeight: 1.6 }}>
                <Stethoscope size={16} color={TEAL} style={{ flexShrink: 0, marginTop: 2 }} />
                <span>可以选泛红、爆痘、暗沉、毛孔里的一个或几个,一起问诊、一起出报告</span>
              </div>
            </div>
            <PrimaryButton onClick={() => goTo("skin")}>
