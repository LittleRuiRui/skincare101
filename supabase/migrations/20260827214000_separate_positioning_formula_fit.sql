...
comment on column public.products.marketing_positioning is 'Concise factual summary of the brand/product marketed target and intended use; not an ingredient efficacy judgment.';
comment on column public.product_formulas.formula_function_summary is 'Evidence-bounded interpretation of what the verified formula plausibly does, independent of brand positioning.';
comment on column public.product_formulas.formula_verdict is 'Short reusable editorial verdict precomputed from verified formula evidence; not personalized to a user.';
...
select p.id,
  ...
  p.name,
  p.brand_local_name,
  p.brand_english_name,
  p.product_local_name,
  p.product_english_name,
  p.source_locale,
  p.category,
  p.market,
  ...
  p.popularity_rank,
  p.popularity_basis,
  p.popularity_sources,
  p.popularity_tier,
  p.asia_availability_status,
  f.id as formula_id,
  f.ingredient_names,
  f.ingredient_list_type,
  f.data_completeness,
  f.verified_at,
  f.formula_dna,
  f.formula_analysis_version,
  f.formula_analyzed_at,
  f.source_last_modified_at,
  f.quality_flags,
  p.marketing_positioning,
  p.intended_skin_types,
  p.intended_concerns,
  p.intended_use_context,
  f.formula_function_summary,
  f.primary_formula_functions,
  f.secondary_formula_functions,
  f.formula_best_for,
  f.formula_also_works_for,
  f.formula_less_ideal_for,
  f.formula_caveats,
  f.formula_verdict,
  f.interpretation_version,
  f.interpreted_at
