...
function V3AppContent() {
  ...
  if(route==="mySkin")return <><style>{FONT_IMPORT}</style>{switcher}<V3MySkin profile={profile} products={products} onBack={()=>setRoute("home")} onRetake={createProfile} onFindProducts={()=>goTo("recommend")} onBuildRoutine={()=>setRoute("routine")} onProduct={(product,concern)=>openProduct(product,concern,"explore")}/></>;
