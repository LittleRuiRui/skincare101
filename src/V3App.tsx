...
import { deleteSkinProfile, loadMySkinProfiles, renameSkinProfile, setActiveSkinProfile } from "./lib/mySkin";
...
function V3AppContent() {
  ...
  useEffect(()=>{let active=true;loadMySkinProfiles().then(result=>{if(active){setProfiles(result);setProfile(result.find(i=>i.isActive)||result[0]||null)}}).catch(()=>{if(active){setProfiles([]);setProfile(null)}}).finally(()=>{if(active)setProfileChecked(true)});loadSharedProductCatalog().then(result=>{if(active)setProducts(result)}).catch(()=>{});return()=>{active=false}},[]);
  useEffect(()=>{async function savePending(){if(pendingProfileSaveRef.current)return;const pending=loadPendingProfileDraftRecord();if(!pending)return;const{data}=await supabase.auth.getSession();if(!data.session?.user)return;pendingProfileSaveRef.current=true;try{await saveMySkinProfile(pending.profile,pending.name||"");clearPendingProfileDraft();await refreshProfiles();setRoute("mySkin")}catch(e){console.error(e)}finally{pendingProfileSaveRef.current=false}}void savePending();const{data}=supabase.auth.onAuthStateChange(event=>{if(event==="INITIAL_SESSION")setTimeout(()=>void savePending(),0);if(event==="SIGNED_IN")setTimeout(()=>void savePending().then(refreshProfiles).then(()=>setRoute("mySkin")),0)});return()=>data.subscription.unsubscribe()},[]);

  async function chooseProfile(profileId:string){const selected=profiles.find(i=>i.id===profileId);if(!selected)return;setProfile(selected);setProfiles(items=>items.map(i=>({...i,isActive:i.id===profileId})));await setActiveSkinProfile(profileId)}
  async function renameProfile(profileId:string,name:string){await renameSkinProfile(profileId,name);await refreshProfiles()}
  async function removeProfile(profileId:string){await deleteSkinProfile(profileId);await refreshProfiles()}
