...
function V3AppContent() {
  ...
  useEffect(()=>{async function savePending(){if(pendingProfileSaveRef.current)return;const pending=loadPendingProfileDraftRecord();if(!pending)return;const{data}=await supabase.auth.getSession();if(!data.session?.user)return;pendingProfileSaveRef.current=true;try{await saveMySkinProfile(pending.profile,pending.name||"我的肤质档案");clearPendingProfileDraft();await refreshProfiles();setRoute("mySkin")}catch(e){console.error(e)}finally{pendingProfileSaveRef.current=false}}void savePending();const{data}=supabase.auth.onAuthStateChange(event=>{if(event==="INITIAL_SESSION")setTimeout(()=>void savePending(),0);if(event==="SIGNED_IN")setTimeout(()=>void savePending().then(refreshProfiles).then(()=>setRoute("mySkin")),0)});return()=>data.subscription.unsubscribe()},[]);
