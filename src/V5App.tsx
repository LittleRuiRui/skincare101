...
export default function V5App(){
 ...
 function submitReport(){
  ...
  const body=[
   "## Product data correction","",`- **Brand:** ${currentProduct.brand}`,`- **Product:** ${currentProduct.productLocalName||currentProduct.name}${currentProduct.productEnglishName&&currentProduct.productEnglishName!==(currentProduct.productLocalName||currentProduct.name)?` / ${currentProduct.productEnglishName}`:""}`,`- **Product ID:** ${currentProduct.id}`,`- **Issue type:** ${typeLabel}`,`- **Current data type:** ${currentProduct.ingredientListType}`,`- **Current source:** ${currentProduct.sourceUrl||"Not listed"}`,`- **Page:** ${window.location.href}`,"","## What looks wrong?",reportNote.trim()||"Please describe the correction here.","","## Suggested source / correct INCI","Please paste a source link, package text, or corrected ingredient list if available.","","---","Submitted from the Skincare101 product feedback form."
  ].join("\n");
