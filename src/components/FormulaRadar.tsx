...
const AXES = [
  ["hydration", "保湿", "Hydration"],
  ["barrier", "屏障", "Barrier"],
  ["soothing", "舒缓", "Soothing"],
  ["antiAging", "抗老", "Anti-aging"],
  ["oilControl", "控油", "Oil control"],
  ["lipid", "脂质", "Lipids"],
] as const;
...
export default function FormulaRadar({ dna }: { dna: FormulaDna }) {
  ...
  const aria=language==="zh"?"配方功效雷达图":"Formula profile radar chart";
  return <div style={{width:"100%",maxWidth:320,margin:"4px auto 8px"}}>
    <svg viewBox="0 0 220 220" role="img" aria-label={aria} style={{width:"100%",display:"block",overflow:"visible"}}>
      {[.25,.5,.75,1].map(s=><polygon key={s} points={ring(s)} fill="none" stroke="#DDD6CA" strokeWidth="1"/>) }
      {AXES.map(([,zh,en],i)=>{const label=language==="zh"?zh:en;const [x,y]=point(i,1);const [tx,ty]=point(i,1.2);return <g key={label}><line x1={cx} y1={cy} x2={x} y2={y} stroke="#E4DED4" strokeWidth="1"/><text x={tx} y={ty} textAnchor="middle" dominantBaseline="middle" fontSize="9" fill="#777065">{label}</text></g>})}
    ...
    <div style={{fontSize:9.5,color:"#777065",lineHeight:1.5,textAlign:"center"}}>{language==="zh"?"0–5 表示配方结构中的相对支持强度，不代表临床功效百分比":"0–5 shows relative structural support in the formula; it is not a clinical efficacy percentage."}</div>
  </div>;
}
