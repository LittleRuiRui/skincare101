...
export default function AnalyticsBootstrap():React.ReactElement|null{
 useEffect(()=>{
  ...
  const onLanguage=(e:Event)=>{const detail=(e as CustomEvent<{language?:AppLanguage}>).detail;trackEventLater("language_changed",{pageName:location.pathname,language:detail?.language||currentLanguage()})};
  ...
  return()=>{window.removeEventListener("skincare101:language",onLanguage as EventListener);document.removeEventListener("click",onClick,true)};
 },[]);
 return null;
}
