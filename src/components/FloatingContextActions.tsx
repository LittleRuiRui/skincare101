...
export default function FloatingContextActions(){
 ...
 const showShelfAction=viewContext==="product"||viewContext==="routine";
 const label=viewContext==="product"?bi("我需要买吗？","Do I need this?"):bi("用我的护肤柜搭配","Use My Shelf");
 ...
 return <>
  {showShelfAction&&<button onClick={action} aria-label={label} style={{position:"fixed",left:14,bottom:"calc(env(safe-area-inset-bottom, 0px) + 16px)",zIndex:120,border:"1px solid #D7CDB8",borderRadius:999,padding:"10px 13px",background:"rgba(251,246,234,.96)",color:"#2F5A40",fontSize:12,fontWeight:700,boxShadow:"0 8px 24px rgba(40,55,42,.12)",cursor:"pointer",backdropFilter:"blur(8px)"}}>{label}</button>}
