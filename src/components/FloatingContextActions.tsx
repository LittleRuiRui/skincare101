...
export default function FloatingContextActions(){
 ...
 return <>
  <button onClick={action} aria-label={label} style={{position:"fixed",left:14,bottom:"calc(env(safe-area-inset-bottom, 0px) + 16px)",zIndex:120,border:"1px solid #D7CDB8",borderRadius:999,padding:"10px 13px",background:"rgba(251,246,234,.96)",color:"#2F5A40",fontSize:12,fontWeight:700,boxShadow:"0 8px 24px rgba(40,55,42,.12)",cursor:"pointer",backdropFilter:"blur(8px)"}}>{label}</button>
  {viewContext==="product"&&currentProduct&&<button onClick={()=>setReportOpen(true)} style={{position:"fixed",right:14,bottom:"calc(env(safe-area-inset-bottom, 0px) + 16px)",zIndex:120,border:"1px solid #D7CDB8",borderRadius:999,padding:"10px 13px",background:"rgba(255,255,255,.96)",color:"#6F6A5F",fontSize:12,fontWeight:700,boxShadow:"0 8px 24px rgba(40,55,42,.10)",cursor:"pointer"}}>发现数据有误？ · Report</button>}
  ...
  {reportOpen&&currentProduct&&<div onClick={()=>setReportOpen(false)} style={{position:"fixed",inset:0,zIndex:180,background:"rgba(27,34,28,.34)",display:"grid",placeItems:"end center",padding:"20px 14px"}}><div onClick={e=>e.stopPropagation()} style={{width:"min(580px,100%)",boxSizing:"border-box",border:"1px solid #D7CDB8",borderRadius:22,padding:18,background:"#FBF6EA",boxShadow:"0 20px 50px rgba(30,40,31,.22)"}}><div style={{fontSize:10.5,fontWeight:800,letterSpacing:".1em",color:"#2F5A40",marginBottom:7}}>DATA CORRECTION</div><div style={{fontSize:22,fontWeight:700,color:"#283027",marginBottom:5}}>发现成分表有问题？</div><div style={{fontSize:12.5,lineHeight:1.6,color:"#6F6A5F",marginBottom:14}}>告诉我哪里不对。产品信息会自动带上，提交后会创建 GitHub 数据纠错记录。</div><label style={{display:"block",fontSize:11.5,fontWeight:700,color:"#4D594F",marginBottom:5}}>问题类型 · Issue type</label><select value={reportType} onChange={e=>setReportType(e.target.value as typeof reportType)} style={{width:"100%",boxSizing:"border-box",border:"1px solid #D7CDB8",borderRadius:12,padding:"10px 11px",background:"#fff",color:"#283027",fontSize:13,marginBottom:11}}>{REPORT_TYPES.map(([value,text])=><option key={value} value={value}>{text}</option>)}</select><label style={{display:"block",fontSize:11.5,fontWeight:700,color:"#4D594F",marginBottom:5}}>哪里不对？· What looks wrong?</label><textarea value={reportNote} onChange={e=>setReportNote(e.target.value)} rows={5} maxLength={1600} placeholder="例如：官网新版成分表已经没有酒精；或贴上正确 INCI / 官方来源链接……" style={{width:"100%",boxSizing:"border-box",resize:"vertical",border:"1px solid #D7CDB8",borderRadius:12,padding:"10px 11px",background:"#fff",color:"#283027",fontSize:13,lineHeight:1.55}}/><div style={{display:"grid",gridTemplateColumns:"1fr 1.5fr",gap:8,marginTop:13}}><button onClick={()=>setReportOpen(false)} style={{border:"1px solid #D7CDB8",borderRadius:13,padding:"10px 11px",background:"white",color:"#6F6A5F",fontWeight:700,cursor:"pointer"}}>取消</button><button onClick={submitReport} style={{border:0,borderRadius:13,padding:"10px 11px",background:"#2F5A40",color:"white",fontWeight:700,cursor:"pointer"}}>提交纠错 · Send report</button></div></div></div>}
 </>;
}
