...
export default function FloatingContextActions(){
 ...
 const showBottomPair=isPrimary;
 ...
 const actionWrap:React.CSSProperties={position:"fixed",left:"50%",bottom:"calc(env(safe-area-inset-bottom, 0px) + 82px)",transform:"translateX(-50%)",zIndex:121,width:"min(86vw,520px)",display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,pointerEvents:"none"};
 const pill=(tone:"shelf"|"need"):React.CSSProperties=>({pointerEvents:"auto",border:`1px solid ${tone==="shelf"?"#D7CDB8":"#E3C978"}`,borderRadius:999,padding:"10px 14px",background:tone==="shelf"?"rgba(255,255,255,.97)":"rgba(255,249,231,.98)",color:tone==="shelf"?"#2F5A40":"#9B741A",fontSize:12,fontWeight:800,boxShadow:"0 10px 28px rgba(40,55,42,.12)",cursor:"pointer",backdropFilter:"blur(10px)",minHeight:48});
 return <>
  {showBottomPair&&<div style={actionWrap}><button onClick={()=>setShelfOpen(true)} style={pill("shelf")}>{t("我的护肤柜","My Shelf")}</button><button onClick={()=>{if(currentProduct)openNeed();else setShelfOpen(true)}} style={pill("need")}>{t("我需要买吗？","Do I need this?")}</button></div>}
