...
function Badge({ children, tone = "neutral" }: { children: React.ReactNode; tone?: "neutral" | "sage" | "rose" }) {
  ...
  return <span style={{ display: "inline-flex", maxWidth:"100%", boxSizing:"border-box", overflowWrap:"anywhere", border: `1px solid ${tone === "neutral" ? LINE : colors[1]}`, borderRadius: 999, padding: "5px 8px", background: colors[0], color: colors[1], fontSize: 10.5 }}>{children}</span>;
}
...
export default function V3ProductDetail({ product, products, profile, concern, onBack, onProduct }: { product: SharedProductRecord; products: SharedProductRecord[]; profile: SkinProfileRecord | null; concern?: BrowseConcern; onBack: () => void; onProduct: (product: SharedProductRecord, concern: BrowseConcern) => void }) {
  ...
  return <div style={{ minHeight: "100vh", width:"100%", maxWidth:"100vw", boxSizing:"border-box", background: PAPER, color: INK, padding: "22px 18px 54px", overflowX:"hidden" }}><div style={{ width:"100%", maxWidth: 620, minWidth:0, boxSizing:"border-box", margin: "0 auto", overflow:"visible" }}>
    ...
    <section style={{ width:"100%", minWidth:0, boxSizing:"border-box", display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 10, marginBottom: 14 }}>
      <div style={{ minWidth:0, boxSizing:"border-box", border: `1px solid ${rating.suitable ? SAGE : rating.stars > 0 ? ROSE : LINE}`, borderRadius: 16, padding: 15, background: rating.suitable ? "#EDF1EA" : rating.stars > 0 ? "#F6ECE8" : "rgba(255,255,255,.65)" }}><div style={{ fontSize: 9.5, color: rating.suitable ? SAGE : rating.stars > 0 ? ROSE : MUTE, letterSpacing: ".08em", marginBottom: 7 }}>MATCH FOR YOU</div>{profile && rating.stars > 0 ? <><div style={{ fontSize: 23, letterSpacing: ".04em", marginBottom: 3 }}>{rating.starsText}</div><div style={{ fontSize: 12.5, fontWeight: 600, color: rating.suitable ? "#4E6254" : "#8E665F", marginBottom: 5 }}>{rating.label}{rating.bonus ? " · Bonus" : ""}</div><div style={{ fontSize: 10.5, color: MUTE, lineHeight: 1.5, marginBottom: 5 }}>{rating.detail}</div><div style={{ fontSize: 9.5, color: MUTE, lineHeight: 1.45 }}>3 星及以上 = 可以使用。星级只表示与你当前肤质和需求的匹配程度。</div></> : <><div style={{ fontSize: 18, marginBottom: 5 }}>{profile ? "证据不足" : "先建立肤质档案"}</div><div style={{ fontSize: 10.5, color: MUTE, lineHeight: 1.5 }}>{profile ? "现有配方证据不足，暂不强行给星级。" : "建立 Skin Profile 后可查看个人适配星级。"}</div></>}</div>
    ...
    <section style={{ width:"100%", minWidth:0, maxWidth:"100%", boxSizing:"border-box", border:`1px solid ${LINE}`, borderRadius:17, padding:17, margin:"18px 0", background:"rgba(255,255,255,.68)", overflow:"hidden" }}><div style={{ fontSize: 9.5, color: SAGE, letterSpacing: ".08em", marginBottom: 6, overflowWrap:"anywhere" }}>{brand.country} · {brand.segment}</div><div style={{ fontFamily: "'Newsreader', serif", fontSize: 22, marginBottom: 6, overflowWrap:"anywhere" }}>About {brand.name}</div><div style={{ fontSize: 11.5, color: MUTE, lineHeight: 1.6, overflowWrap:"anywhere" }}>{brand.description}</div><div style={{ fontSize: 11.5, marginTop: 9, overflowWrap:"anywhere" }}><b>Known for:</b> {brand.knownFor}</div><div style={{ fontSize: 11.5, marginTop: 5, overflowWrap:"anywhere" }}><b>Best for:</b> {brand.bestFor}</div><div style={{ fontSize: 11.5, marginTop: 5, overflowWrap:"anywhere" }}><b>Price:</b> {approximatePriceGuide(brand, product.category)}</div><div style={{ fontSize: 9.5, color: MUTE, lineHeight: 1.5, marginTop: 4 }}>这是帮助筛选预算的估算区间，不是实时售价；促销、容量和零售渠道会造成差异。</div></section>
