...
export function LanguageSwitch() {
  ...
  return <div style={{display:"inline-flex",border:"1px solid #E4E0D8",borderRadius:999,padding:3,background:"rgba(252,251,248,.96)",boxShadow:"0 2px 10px rgba(40,38,32,.04)"}} aria-label="Language selector">{(["zh","en"] as AppLanguage[]).map(item=><button key={item} onClick={()=>setLanguage(item)} style={{border:0,borderRadius:999,padding:"6px 9px",background:language===item?"#66786E":"transparent",color:language===item?"#fff":"#817D75",fontSize:11,fontWeight:650,cursor:"pointer"}}>{item==="zh"?"中文":"EN"}</button>)}</div>;
}
